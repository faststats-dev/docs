...
Add the error-tracking extension to capture uncaught errors and unhandled
promise rejections. The tracker batches them and drops noise from browser
...
When you catch an error yourself, send it through the running client. An
error-tracking extension must be active to receive the report.
...
In React, get the client with `useAnalytics()` and call
`analytics?.reportError(error)`. In Nuxt, call `$faststats.reportError(error)`.
