...
function Handle({ side }: { side: "top" | "bottom" }) {
	return (
		<div
			className={`bg-muted-foreground/70 border-card absolute left-1/2 size-2 -translate-x-1/2 rounded-full border ${side === "top" ? "-top-1" : "-bottom-1"
				}`}
		/>
	);
}
...
function FakeInput({
	...
	return (
		<div
			className={`border-input flex h-7 w-full items-center border bg-transparent px-2 text-xs ${mono ? "font-mono" : ""
...
function FakeSelect({
	...
	return (
		<div
			className={`border-input flex h-7 w-full items-center justify-between border bg-transparent px-2 text-xs ${mono ? "font-mono" : ""
				}`}
		>
...
function FakeCheckbox({
	...
	return (
		<div className="flex items-center gap-2 px-1 py-0.5">
			<div
				className={`flex size-3.5 shrink-0 items-center justify-center border ${checked
